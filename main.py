...
def scrape(destination, number_of_days):
    ...
    for check_in_date, check_out_date in dates:
        ...
        hotels_page.load_hotels_page(chrome_driver, timer, destination, check_in_date, check_out_date)
        hotels_count.get_total_hotels_count(chrome_driver, timer)
        available_hotels = hotels_count.get_available_hotels_count(chrome_driver, timer)
        hotel_data = hotels_detail.get_hotel_data(chrome_driver, timer, destination, check_in_date, check_out_date, available_hotels)
        file_name = file.generate_file_name(destination, check_in_date, check_out_date)
